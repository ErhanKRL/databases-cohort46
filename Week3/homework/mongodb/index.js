...
async function createEpisodeExercise(episodesCollection) {
  /**
  ...
  // Write code that will add this to the collection!
    ...
    const result = await episodesCollection.insertOne(episodeData);
...
async function findEpisodesExercises(episodesCollection) {
  /**
  ...
  const cliffAndLighthouseEpisodes = await episodesCollection.find({ elements: { $all: ["CLIFF", "LIGHTHOUSE"] } }).toArray();
  if(cliffAndLighthouseEpisodes.length === 0){
    throw new Error(`There is no "CLIFF and a LIGHTHOUSE" painting`);
  }
  const cliffAndLighthouseEpisodesTitles = cliffAndLighthouseEpisodes.map(episode => episode.title);
  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE is ${cliffAndLighthouseEpisodesTitles.join(', ')}`
  );
}

async function updateEpisodeExercises(episodesCollection) {
  /**
  ...
  const result = await episodesCollection.updateOne({ episode: "S30E13" }, { $set: { title: "BLUE RIDGE FALLS" } });
  console.log(
  ...
  const updateResult = await episodesCollection.updateMany({ elements: "BUSHES" }, { $set: { "elements.$": "BUSH" } });
  console.log(
...
async function deleteEpisodeExercise(episodesCollection) {
  /**
...
async function main() {
  ...
  try {
    ...
    // CREATE
    await createEpisodeExercise(episodesCollection);

    // READ
    await findEpisodesExercises(episodesCollection);

    // UPDATE
    await updateEpisodeExercises(episodesCollection);

    // DELETE
    await deleteEpisodeExercise(episodesCollection);
  } catch (err) {
    console.error(err);
  } finally {
